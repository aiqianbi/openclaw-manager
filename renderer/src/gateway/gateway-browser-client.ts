...
export class GatewayBrowserClient {
  ...
  private buildConnectClient(): GatewayConnectClientInfo {
    return {
      ...
      version: this.opts.clientVersion ?? "openclaw-manager",
  ...
  private async sendConnect() {
    ...
    void this.request<GatewayHelloOk>("connect", this.buildConnectParams(plan))
      ...
      .catch((failure: unknown) => this.handleConnectFailure(failure, plan));
  }
  ...
  private handleMessage(raw: string) {
    ...
    if (frame.type === "event") {
      ...
      try {
        ...
        console.error("[gateway] event handler error:", handlerErr);
      }
      return;
    }
